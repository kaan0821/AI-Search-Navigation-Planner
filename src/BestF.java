...
public class BestF extends GeneralSearch {
    //Priority Queue
    ...
    protected void expand(Node curr_node, Boolean upward) {
        ...
        if (c < (columns-1) && m[r][c+1] != 1) {
            ...
            Node rightNode = new Node(new Coord(r, c+1), curr_node, 1,fcost,depth);
        ...
        if (upward) {
            if (r < (rows-1) && m[r+1][c] != 1) {
                ...
                Node downNode = new Node(new Coord(r+1, c), curr_node, 2,fcost,depth);
        ...
        if (c > 0 && m[r][c-1] != 1) {
            fcost = computeDistance(r, c-1, adjacent, rg, cg, dirg);
            Node leftNode = new Node(new Coord(r, c-1), curr_node, 3,fcost,depth);
        ...
        if (!upward) {
            if (r > 0 && m[r-1][c] != 1) {
                fcost = computeDistance(r-1, c, adjacent, rg, cg, dirg);
                Node upNode = new Node(new Coord(r-1, c), curr_node, 4,fcost,depth);
